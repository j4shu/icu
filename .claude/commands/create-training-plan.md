...
Build a training plan that runs from today until the athlete's next race. Arguments (`$ARGUMENTS`) may give how many days of history to consider — if none is given, use 42 (long enough to judge periodization).
...
4. **Every week must include this fixed set of sessions:**
   - **2 key/hard sessions** for bike and run each week. For bike, this could be sweet spot/threshold intervals, vo2 max intervals, etc. For run, this scould be tempo runs, hill repeats, intervals, etc. It can also be a brick session that combines bike and run.
   - **1 strength session.**
   - **2 swim sessions.**
   - **1 rest day.**
...
7. Run `date "+%Y-%m-%d-%H%M"` (e.g. `2026-05-31-0925`) and write the plan to `training-plans/<that-timestamp>-<race-slug>.md` (use dashes in the race slug). Do **not** overwrite `training_plan.md`.
